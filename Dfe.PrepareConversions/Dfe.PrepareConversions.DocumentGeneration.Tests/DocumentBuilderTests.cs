...
namespace Dfe.PrepareConversions.DocumentGeneration.Tests
{
	public class DocumentBuilderTests
	{
		...
		public class ParagraphTests : DocumentBuilderTests
		{
			...
			{
				var text = new[] { new TextElement { Value = "Meow" }, new TextElement { Value = "Woof", Bold = true }, new TextElement { Value = "Quack" } };
		...
		public class TableTests : DocumentBuilderTests
		{
			...
			{
				var textElements = new[] { new TextElement { Value = "test1" }, new TextElement { Value = "test2" } };
		...
		public class DocumentFromTemplateTests : DocumentBuilderTests
		{
			...
			public void GivenTemplateDocumentWithRichTextPlaceholder_PopulatesRichText()
			{
				...
				var template = documentBuilder.Build();
				var document = new ExampleDocument() { RichText = "<p>Some rich text</p><p>Goes here</p>" };

				using var ms = new MemoryStream();
				ms.Write(template);
				
				var builderFromTemplate = DocumentBuilder.CreateFromTemplate(ms, document);
				var result = builderFromTemplate.Build();

				var createdDocument = WordprocessingDocument.Open(new MemoryStream(result), false);
				var paragraphs = createdDocument.MainDocumentPart.Document.Body.Descendants<Paragraph>().ToList();
