...
namespace DocumentGeneration.Helpers
{
	public static class HtmlToElements
	{
		...
		{
			var documentElements = new List<OpenXmlElement>();
			...
			foreach (var elementList in res)
			{
				switch (elementList[0])
				{
					...
					case "<div>":
						...
						break;
					case "<ul>":
						var bulletedListBuilder = new BulletedListBuilder(builder.GetCurrentDocument().MainDocumentPart.NumberingDefinitionsPart);
						BuildListFromElements(bulletedListBuilder, elementList.Skip(1).ToList());
						documentElements.AddRange(bulletedListBuilder.Build());
						break;
					case "<ol>":
						var numberedListBuilder = new NumberedListBuilder(builder.GetCurrentDocument().MainDocumentPart.NumberingDefinitionsPart);
						BuildListFromElements(numberedListBuilder, elementList.Skip(1).ToList());
						documentElements.AddRange(numberedListBuilder.Build());
						break;
					default:
						var defaultPBuilder = new ParagraphBuilder();
