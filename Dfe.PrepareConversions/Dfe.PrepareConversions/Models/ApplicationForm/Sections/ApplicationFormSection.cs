...
public class ApplicationFormSection : BaseFormSection
{
   ...
   {
      List<FormField> formFields = new() { new("Trust name", application.TrustName) };
      formFields.Add(new FormField("Will there be any changes to the governance of the trust due to the school joining?",
         application.ChangesToTrust == null ? "Unknown" : application.ChangesToTrust.ToYesNoString()));
      if (application.ChangesToTrust == true) formFields.Add(new FormField("What are the changes?", application.ChangesToTrustExplained));
      formFields.Add(new FormField("Will there be any changes at a local level due to this school joining?",
         application.ChangesToLaGovernance == null ? "Unknown" : application.ChangesToLaGovernance.ToYesNoString()));
      if (application.ChangesToLaGovernance == true)
         formFields.Add(
