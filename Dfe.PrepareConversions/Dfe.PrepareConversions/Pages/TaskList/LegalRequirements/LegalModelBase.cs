...
namespace Dfe.PrepareConversions.Pages.TaskList.LegalRequirements
{
	public class LegalModelBase : PageModel
	{
		...
		public Dfe.PrepareConversions.Data.Models.AcademyConversion.LegalRequirements Requirements { get; private set; }
		...
		public override async Task OnPageHandlerExecutionAsync(PageHandlerExecutingContext context, PageHandlerExecutionDelegate next)
		{
			if (context.HandlerArguments.ContainsKey(nameof(Id)))
			{
				...
				if (project.Success)
				{
					Requirements = Dfe.PrepareConversions.Data.Models.AcademyConversion.LegalRequirements.From(project.Body);
				}
				else
				{
					context.Result = NotFound();
