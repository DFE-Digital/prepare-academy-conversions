...
export default class Performance extends BasePage {
  ...
  static keyStagePath = (keyStageNumber: number): string =>
    `key-stage-${keyStageNumber}-performance-tables`;

  static verifyKeyStageScreenText(urn: string): void {
    cy.get(this.selectors.keyStageTextId).should(
      'contain.text',
      this.selectors.keyStageText
    );
    cy.get(this.selectors.keyStageLinkTextId).should(
      'have.text',
      this.selectors.keyStageLinkText
    );
  ...
  static verifyOfsteadScreenText(): void {
    cy.get(this.selectors.ofstedInfoTextId).should(
      'contain.text',
      this.selectors.ofstedInfoText
    );
  }
