...
export default class voluntaryProjectTaskList extends BasePage {
  static voluntaryProjectElementsVisible(): void {
    ...
    const projectOwner = '';
    const projectStatus = 'Approved with Conditions';

    // Check Header section of main Project Page
    cy.get('h1[data-cy]').contains(projectName);
    cy.get('p[class="govuk-body govuk-!-margin-bottom-1"]').contains(
      'Route: ' + routeType
    );
    cy.get('p[class="govuk-body govuk-!-margin-bottom-5"]').contains(
