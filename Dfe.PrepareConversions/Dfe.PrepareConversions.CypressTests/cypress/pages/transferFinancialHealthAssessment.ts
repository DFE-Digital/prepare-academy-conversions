...
class TransferFinancialHealthAssessment extends BasePage {
    ...
    private readonly selectors = {
        ...
        overviewInput: 'sfso-commissioning-overview', // data-test on the textarea
    ...
    public getOverview(): Cypress.Chainable<JQuery<HTMLElement>> {
        cy.checkPath(this.path);
        return cy.getByDataTest(this.selectors.overviewInput);
    }

    public enterOverview(text: string): this {
        cy.checkPath(this.path);
        cy.getByDataTest(this.selectors.overviewInput).clear();
        if (text.length > 0) {
            cy.getByDataTest(this.selectors.overviewInput).type(text, { delay: 0 });
        }
        return this;
    }
