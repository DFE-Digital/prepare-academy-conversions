...
describe('Sponsored conversion journey', {}, () => {
   ...
   const testData = {
      ...
      psed: {
         unlikely:
            'The decision is unlikely to disproportionately affect any particular person ',
         someImpact:
            'There are some impacts but on balance the analysis indicates these changes',
   ...
   it('User could Assign Project', () => {
      ...
      projectAssignmentConversion.assignProject(
         testData.projectAssignment.deliveryOfficer
      );
      projectTaskList
      ...
      projectTaskList
         .getAssignedUser()
         .should('contain.text', testData.projectAssignment.deliveryOfficer);
      projectList.filterProjectList(testData.projectName);
      projectList
         .getNthProjectDeliveryOfficer()
         .should('contain.text', testData.projectAssignment.deliveryOfficer);
   });
   ...
   it('User could complete the School Overview task ', () => {
      ...
      schoolOverview
         .getPFIDetails()
         .should('contain.text', testData.schoolOverview.pfiDescription);
      //Distance plus details
      schoolOverview.changeDistance(
         testData.schoolOverview.distance,
         testData.schoolOverview.distanceDecription
      );
      schoolOverview
         .getDistance()
         .should('contain.text', testData.schoolOverview.distance);
      schoolOverview
         .getDistanceDetails()
         .should('contain.text', testData.schoolOverview.distanceDecription);
      //Complete
      schoolOverview.markComplete();
      cy.confirmContinueBtn().click();
      projectTaskList
         .getSchoolOverviewStatus()
         .should('contain.text', testData.completedText);
   });
   ...
   it('User could complete Budget task ', () => {
      ...
      budget
         ...
         .should('contain.text', testData.budget.endOfFinanicalYear.getDate());
      budget
         .getCurrentFinancialYear()
         .should(
            'contain.text',
      ...
      budget
         .getCurrentFinancialYear()
         .should('contain.text', testData.budget.endOfFinanicalYear.getFullYear());
      budget
         .getCurrentRevenue()
         .should('contain.text', `£${testData.budget.forecastedRevenueCurrentYear}`);
      budget
         .getCurrentCapital()
         .should('contain.text', `£${testData.budget.forecastedCapitalCurrentYear}`);

      budget
         .getNextFinancialYear()
         .should('contain.text', testData.budget.endOfNextFinancialYear.getDate());
      budget
         .getNextFinancialYear()
         .should(
            'contain.text',
      ...
      budget
         ...
         .should(
            'contain.text',
            testData.budget.endOfNextFinancialYear.getFullYear()
         );
      budget
         .getNextRevenue()
         .should('contain.text', `£${testData.budget.forecastedRevenueNextYear}`);
      budget
         .getNextCapital()
         .should('contain.text', `£${testData.budget.forecastedCapitalNextYear}`);
   ...
   it('User could complete Pupil Forecast task', () => {
      ...
      PupilForecast.enterAditionalInfomation(
         testData.pupilForecast.additionalInfomation
      );
      PupilForecast.getAdditionalInfomation().should(
         'contain.text',
         testData.pupilForecast.additionalInfomation
      );
      cy.confirmContinueBtn().click();
   ...
   it('User gets an error message whilst previewing or creating project document for incomplete PSED task', () => {
      ...
      projectTaskList
         .getErrorMessage()
         .should('include.text', testData.psed.errorMessage);
   ...
   it('User could complete Public Sector Equality Duty task for Unlikely Impact', () => {
      ...
      PublicSectorEqualityDutyImpact.getPsed().should(
         'include.text',
         testData.psed.unlikely
      );
      PublicSectorEqualityDutyImpact.markComplete();
      cy.confirmContinueBtn().click();
      projectTaskList
         .publicSectorEqualityDutyStatus()
         .should('contain.text', testData.completedText);
   });
   ...
   it('User could complete Public Sector Equality Duty task for Some Impact', () => {
      ...
      PublicSectorEqualityDutyImpact.getErrorMessage().should(
         'include.text',
         testData.psed.reasonErrorMessage
      );
      //should be able to save with impact reason
      PublicSectorEqualityDutyImpact.enterImpactReason();
      PublicSectorEqualityDutyImpact.getPsed().should(
         'include.text',
         testData.psed.someImpact
      );
      PublicSectorEqualityDutyImpact.getImpactReason().should(
         'include.text',
         testData.psed.reason
      );
      PublicSectorEqualityDutyImpact.markComplete();
      cy.confirmContinueBtn().click();
      projectTaskList
   ...
   it('User could complete Risks and issues task', () => {
      ...
      RisksAndIssues.getRisksAndIssues().should(
         'contain.text',
         testData.risksAndIssues
      );
      RisksAndIssues.markComplete();
      cy.confirmContinueBtn().click();
      projectTaskList
         .getRisksAndIssuesStatus()
         .should('contain.text', testData.completedText);
   });
   ...
   it('User could complete Local authority task ', () => {
      ...
      LocalAuthorityInfomation.getComments().should(
         'contain.text',
         testData.localAuthority.comment
      );

      LocalAuthorityInfomation.changeSharepointLink(
         testData.localAuthority.sharepointLink
      );
      LocalAuthorityInfomation.getSharepointLink().should(
         'contain.text',
         testData.localAuthority.sharepointLink
      );
