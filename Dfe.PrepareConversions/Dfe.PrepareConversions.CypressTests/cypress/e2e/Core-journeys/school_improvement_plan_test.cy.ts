...
describe('School Improvement Plan Tests', () => {
   ...
   it('Creating a conversion project and recording a new interim school improvement plan', () => {
      ...
      schoolImprovementPage
         .searchSchool('Manchester Academy (134224)')
         .clickContinue();
      ...
      cy.url()
         .then((url) => {
            const match = url.match(/task-list\/(\d+)/);
            projectId = match ? match[1] : '';
            Logger.log(`Project ID: ${projectId}`);
         })
         .then(() => {
            Logger.log(
               'Navigate to the section to record interim school improvement plan'
            );
            ...
            Logger.log(
               'Enter the who is providing the interim school improvement plan details'
            );
            ...
            Logger.log(
               'Verify the interim school improvement plan details are correct'
            );
            ...
            Logger.log(
               'Verify the submitted interim school improvement plan details are saved correctly'
            );
            schoolImprovementPage.verifyTheFinalImprovementDetails(
               'Local Authority',
               'Fahad-Cypress Test',
               '12 December 2023',
               '12 November 2024',
            ...
            schoolImprovementPage.changeImprovementDetails(
               'Cypress Test Interim schoool plan',
               'Test comment'
            );
            Logger.log(
               'Verify the changed interim school improvement plan details are saved correctly'
            );
            ...
            Logger.log(
               'Delete the project and verify that it was deleted successfully - Project ID: ' +
               projectId
            );
