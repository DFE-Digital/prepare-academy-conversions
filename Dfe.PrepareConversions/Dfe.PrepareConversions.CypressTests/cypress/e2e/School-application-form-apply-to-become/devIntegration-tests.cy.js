    ...
    it('TC11: Next Financial Year', () => {
        ...
        cy.get('[test-id="Finances_Next_financial_year4_value"]')
            ...
            .then((text) => {
                let textvalue = text.toString()
                if (textvalue.length > 0) {
                    expect(data.applyingSchools[0].nextFinancialYear.revenueStatusExplained)
                        .to.equal(textvalue.trim())
                }
                else {
                    return null
        ...
        cy.get('[test-id="Finances_Next_financial_year7_value"]')
            ...
            .then((text) => {
                let textvalue = text.toString()
                if (textvalue.length > 0) {
                    expect(data.applyingSchools[0].nextFinancialYear.capitalStatusExplained)
                        .to.equal(textvalue.trim())
                }
                else {
                    return null
    ...
    it('TC14: Financial Investigations', () => {
        ...
        cy.get('[test-id="Finances_Financial_investigations2_value"]')
            ...
            .then((text) => {
                let textvalue = text.toString()
                if (textvalue.length > 0) {
                    expect(data.applyingSchools[0].schoolFinancialInvestigationsExplain)
                        .to.equal(text.trim())
                }
                else {
                    return null
        ...
        cy.get('[test-id="Finances_Financial_investigations3_value"]')
            ...
            .then((text) => {
                let textvalue = text.toString()
                if (textvalue == 'Yes' || 'No') {
                    expect(data.applyingSchools[0].schoolFinancialInvestigationsTrustAware)
                        .to.be.a('boolean')
                }
                else {
                    return null
