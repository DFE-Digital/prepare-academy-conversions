...
namespace ApplyToBecomeInternal.Tests.Pages.TaskList.LegalRequirements
{
	public class FoundationConsentTests : LegalRequirementsPageTestBase
	{
		...
		private string SummaryFoundationConsentStatus => Document.QuerySelector(CypressSelectorFor(Legal.Summary.FoundationConsent.Status))?.Text().Trim();

		[Fact]
		public void Should_have_a_back_link_that_points_to_the_legal_summary_page()
		{
			BackLinkHref.Should().EndWith($"/task-list/{Project.Id}/legal-requirements/legal-summary");
		}
		...
		public async Task Should_not_update_the_summary_status_if_an_option_was_not_selected()
		{
			...
			SummaryFoundationConsentStatus.Should().BeEquivalentTo("empty");
		}
	}
}
