...
namespace ApplyToBecomeInternal.Tests.Pages.PreviewHTBTemplate
{
	public class PreviewHTBIntegrationTests : BaseIntegrationTests
	{
		...
		public async Task Should_display_general_information_section()
		{
			...
			await OpenUrlAsync($"/task-list/{project.Id}/preview-advisory-board-template");
			Document.Url.Should().Contain($"/task-list/{project.Id}/preview-advisory-board-template");
		...
		public async Task Should_navigate_to_school_and_trust_prev_htb_page_and_back()
		{
			...
			Document.Url.Should().Contain($"/task-list/{project.Id}/confirm-school-trust-information-project-dates/previous-advisory-board-date-question");
