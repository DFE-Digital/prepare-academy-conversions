...
Cypress._.each(['iphone-x'], (viewport) => {
	describe(`86341: Error messaging should be correct on ${viewport}`, () => {
		...
		context("when form submitted", () => {
			...
			it.skip('TC02: Should display "Advisory board date must be in the future" when an elapsed date has been submitted', () => {
				cy.url().then(href => {
			...
			it.skip('TC03: Should display "Advisory Board must be a valid date" when submitting invalid month', () => {
